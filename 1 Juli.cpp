...
int main()
{
    ...
    string data4, Dt_stringline[100];
    char filename1IN[20],filenameOUT[20],filename2IN[20];
    float data1, data2, data3,data5,Dt_list[100][2],Dt_list2[100],Dt_list3[100];
    int N,i=0;
    ...
    infile.open(filename1IN);
	if (!infile.is_open()){
    	cout << "\n\n\n File " << filename1IN <<" does not exist" <<endl;
    ...
    //if the file does not exist then show this comment
    	...
    	cout << "File " << filename1IN <<" does not exist" <<endl;
		cout <<"Unable to be opened.";
	}
    N=16;
    float n = N;
  ...
  for (i=0;i<n;i++)                                                                                             //output matriks y//
    ...
    //ifstream infile;
    //specified a file name to be open
    infile.open(filename2IN);
	if (!infile.is_open()){
    	cout << "\n\n\n File " << filename2IN <<" does not exist" <<endl;
		cout <<"Unable to be opened.\n\n\n\n";
		return(0);
	} else {
	       // input filename to be opened
    ...
    if (infile.is_open())
    {
        ...
        while ( !infile.eof())
        {
            infile >> data3 >> data4 >> data5;
			//print data to the screen and save it in an Array Dt_stringline
            cout << data3 <<"  "<< data4 <<"  "<< data5 << '\n';
            Dt_list2[i]=data3;
    ...
    //if the file does not exist then show this comment
    	...
    	cout << "File " << filename2IN <<" does not exist" <<endl;
		cout <<"Unable to be opened.";
	}
